...
func (s *Service) List(ctx context.Context, req ListRequest) (*ListResponse, error) {
	...
	var bankAccounts []map[string]any
	resp, err := httpClient.GetJSON(ctx, "/bank_accounts.json", query, &bankAccounts)
	if err != nil {
		return nil, err
	}
	return &ListResponse{
		...
		Pagination: output.Pagination{
			Page:     req.Page,
			PerPage:  req.PerPage,
			Returned: len(bankAccounts),
			HasNext:  len(bankAccounts) == req.PerPage,
		},
	}, nil
}
...
func (s *Service) Get(ctx context.Context, req GetRequest) (*GetResponse, error) {
	if strings.TrimSpace(req.ID) == "" {
		return nil, output.Usage("missing_id", "bank account ID is required", "pass --id <bank-account-id>")
	}
	...
	var bankAccount map[string]any
	resp, err := httpClient.GetJSON(ctx, fmt.Sprintf("/bank_accounts/%s.json", url.PathEscape(req.ID)), nil, &bankAccount)
	if err != nil {
		return nil, err
	}
	return &GetResponse{
...
func (s *Service) Create(ctx context.Context, req CreateRequest) (*CreateResponse, error) {
	...
	if req.DryRun {
		plan := transport.PlanJSONRequest(http.MethodPost, "/bank_accounts.json", nil, payload)
		return &CreateResponse{Profile: resolved.Name, DryRun: &plan}, nil
	}

	var bankAccount map[string]any
	resp, err := httpClient.PostJSON(ctx, "/bank_accounts.json", payload, &bankAccount)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, req UpdateRequest) (*UpdateResponse, error) {
	...
	path := fmt.Sprintf("/bank_accounts/%s.json", url.PathEscape(req.ID))
	payload := wrapPayload(req.Input)
	if req.DryRun {
	...
	var bankAccount map[string]any
	resp, err := httpClient.PutJSON(ctx, path, payload, &bankAccount)
	if err != nil {
		return nil, err
	}
...
func validateInput(input map[string]any) error {
	if input == nil {
		return output.Usage("missing_input", "bank account input is required", "pass --input -|@file.json|'{...}'")
	}
	return nil
}
...
func wrapPayload(input map[string]any) map[string]any {
	return map[string]any{
		"bank_account": cloneValue(input).(map[string]any),
	}
}
