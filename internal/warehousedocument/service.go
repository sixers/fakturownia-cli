...
func (s *Service) List(ctx context.Context, req ListRequest) (*ListResponse, error) {
	...
	var warehouseDocuments []map[string]any
	resp, err := httpClient.GetJSON(ctx, "/warehouse_documents.json", query, &warehouseDocuments)
	if err != nil {
		return nil, err
	}
	return &ListResponse{
		...
		Pagination: output.Pagination{
			Page:     req.Page,
			PerPage:  req.PerPage,
			Returned: len(warehouseDocuments),
			HasNext:  len(warehouseDocuments) == req.PerPage,
		},
	}, nil
}
...
func (s *Service) Get(ctx context.Context, req GetRequest) (*GetResponse, error) {
	if strings.TrimSpace(req.ID) == "" {
		return nil, output.Usage("missing_id", "warehouse document ID is required", "pass --id <warehouse-document-id>")
	}
	...
	var warehouseDocument map[string]any
	resp, err := httpClient.GetJSON(ctx, fmt.Sprintf("/warehouse_documents/%s.json", url.PathEscape(req.ID)), nil, &warehouseDocument)
	if err != nil {
		return nil, err
	}
	return &GetResponse{
...
func (s *Service) Create(ctx context.Context, req CreateRequest) (*CreateResponse, error) {
	...
	if req.DryRun {
		plan := transport.PlanJSONRequest(http.MethodPost, "/warehouse_documents.json", nil, payload)
		return &CreateResponse{Profile: resolved.Name, DryRun: &plan}, nil
	}

	var warehouseDocument map[string]any
	resp, err := httpClient.PostJSON(ctx, "/warehouse_documents.json", payload, &warehouseDocument)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, req UpdateRequest) (*UpdateResponse, error) {
	...
	path := fmt.Sprintf("/warehouse_documents/%s.json", url.PathEscape(req.ID))
	payload := wrapPayload(req.Input)
	if req.DryRun {
	...
	var warehouseDocument map[string]any
	resp, err := httpClient.PutJSON(ctx, path, payload, &warehouseDocument)
	if err != nil {
		return nil, err
	}
...
func validateInput(input map[string]any) error {
	if input == nil {
		return output.Usage("missing_input", "warehouse document input is required", "pass --input -|@file.json|'{...}'")
	}
	return nil
}
...
func wrapPayload(input map[string]any) map[string]any {
	return map[string]any{
		"warehouse_document": cloneValue(input).(map[string]any),
	}
}
