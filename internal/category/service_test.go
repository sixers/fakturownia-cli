...
func TestListBuildsQueryAndPagination(t *testing.T) {
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode([]map[string]any{{"id": 100, "name": "my_category"}})
	}))
	defer server.Close()
...
func TestGetReturnsCategory(t *testing.T) {
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/categories/100.json" {
	...
	if result.Category["name"] != "my_category" {
...
func TestCreateUpdateDeleteAndDryRun(t *testing.T) {
	...
	created, err := service.Create(context.Background(), CreateRequest{
		...
		Timeout:    5 * time.Second,
		MaxRetries: 1,
		Input:      map[string]any{"name": "my_category", "description": nil},
	})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	if created.Category["id"] == nil {
		t.Fatalf("unexpected create response: %#v", created.Category)
	}
	...
	if updated.Category["description"] != "new_description" {
		t.Fatalf("unexpected update response: %#v", updated.Category)
	}
