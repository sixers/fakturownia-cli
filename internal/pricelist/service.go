...
func (s *Service) List(ctx context.Context, req ListRequest) (*ListResponse, error) {
	...
	var priceLists []map[string]any
	resp, err := httpClient.GetJSON(ctx, "/price_lists.json", query, &priceLists)
	if err != nil {
		return nil, err
	}
	return &ListResponse{
		...
		Pagination: output.Pagination{
			Page:     req.Page,
			PerPage:  req.PerPage,
			Returned: len(priceLists),
			HasNext:  len(priceLists) == req.PerPage,
		},
	}, nil
}
...
func (s *Service) Get(ctx context.Context, req GetRequest) (*GetResponse, error) {
	if strings.TrimSpace(req.ID) == "" {
		return nil, output.Usage("missing_id", "price list ID is required", "pass --id <price-list-id>")
	}
	...
	resp, err := httpClient.GetJSON(ctx, fmt.Sprintf("/price_lists/%s.json", url.PathEscape(req.ID)), nil, &priceList)
	if err != nil {
		return nil, err
	}
	return &GetResponse{
...
func (s *Service) Create(ctx context.Context, req CreateRequest) (*CreateResponse, error) {
	...
	if req.DryRun {
		plan := transport.PlanJSONRequest(http.MethodPost, "/price_lists.json", nil, payload)
		return &CreateResponse{Profile: resolved.Name, DryRun: &plan}, nil
	}

	var priceList map[string]any
	resp, err := httpClient.PostJSON(ctx, "/price_lists.json", payload, &priceList)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, req UpdateRequest) (*UpdateResponse, error) {
	...
	path := fmt.Sprintf("/price_lists/%s.json", url.PathEscape(req.ID))
	payload := wrapPayload(req.Input)
	if req.DryRun {
	...
	var priceList map[string]any
	resp, err := httpClient.PutJSON(ctx, path, payload, &priceList)
	if err != nil {
		return nil, err
	}
...
func validateInput(input map[string]any) error {
	if input == nil {
		return output.Usage("missing_input", "price list input is required", "pass --input -|@file.json|'{...}'")
	}
	return nil
}
...
func wrapPayload(input map[string]any) map[string]any {
	return map[string]any{
		"price_list": cloneValue(input).(map[string]any),
	}
}
