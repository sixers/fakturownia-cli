...
func (s *Service) List(ctx context.Context, req ListRequest) (*ListResponse, error) {
	...
	var warehouses []map[string]any
	resp, err := httpClient.GetJSON(ctx, "/warehouses.json", query, &warehouses)
	if err != nil {
		return nil, err
	}
	return &ListResponse{
		...
		Pagination: output.Pagination{
			...
			HasNext:  len(warehouses) == req.PerPage,
		},
	}, nil
}
...
func (s *Service) Get(ctx context.Context, req GetRequest) (*GetResponse, error) {
	if strings.TrimSpace(req.ID) == "" {
		return nil, output.Usage("missing_id", "warehouse ID is required", "pass --id <warehouse-id>")
	}
	...
	var warehouse map[string]any
	resp, err := httpClient.GetJSON(ctx, fmt.Sprintf("/warehouses/%s.json", url.PathEscape(req.ID)), nil, &warehouse)
	if err != nil {
		return nil, err
	}
	return &GetResponse{
...
func (s *Service) Create(ctx context.Context, req CreateRequest) (*CreateResponse, error) {
	...
	if req.DryRun {
		plan := transport.PlanJSONRequest(http.MethodPost, "/warehouses.json", nil, payload)
		return &CreateResponse{Profile: resolved.Name, DryRun: &plan}, nil
	}

	var warehouse map[string]any
	resp, err := httpClient.PostJSON(ctx, "/warehouses.json", payload, &warehouse)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, req UpdateRequest) (*UpdateResponse, error) {
	...
	path := fmt.Sprintf("/warehouses/%s.json", url.PathEscape(req.ID))
	payload := wrapPayload(req.Input)
	if req.DryRun {
	...
	var warehouse map[string]any
	resp, err := httpClient.PutJSON(ctx, path, payload, &warehouse)
	if err != nil {
		return nil, err
	}
...
func validateInput(input map[string]any) error {
	if input == nil {
		return output.Usage("missing_input", "warehouse input is required", "pass --input -|@file.json|'{...}'")
	}
	return nil
}
...
func wrapPayload(input map[string]any) map[string]any {
	return map[string]any{
		"warehouse": cloneValue(input).(map[string]any),
	}
}
