...
func (s *Service) List(ctx context.Context, req ListRequest) (*ListResponse, error) {
	...
	resp, err := client.GetJSON(ctx, "/issuers.json", query, &issuers)
	if err != nil {
		return nil, err
	}
	return &ListResponse{
		...
		Pagination: output.Pagination{
			Page:     req.Page,
			PerPage:  req.PerPage,
			Returned: len(issuers),
			HasNext:  len(issuers) == req.PerPage,
		},
	}, nil
}
...
func (s *Service) Get(ctx context.Context, req GetRequest) (*GetResponse, error) {
	if strings.TrimSpace(req.ID) == "" {
		return nil, output.Usage("missing_id", "issuer ID is required", "pass --id <issuer-id>")
	}
	...
	resp, err := client.GetJSON(ctx, fmt.Sprintf("/issuers/%s.json", url.PathEscape(req.ID)), nil, &issuer)
	if err != nil {
		return nil, err
	}
	return &GetResponse{
...
func (s *Service) Create(ctx context.Context, req CreateRequest) (*CreateResponse, error) {
	...
	if req.DryRun {
		plan := transport.PlanJSONRequest(http.MethodPost, "/issuers.json", nil, payload)
		return &CreateResponse{Profile: resolved.Name, DryRun: &plan}, nil
	}

	var issuer map[string]any
	resp, err := client.PostJSON(ctx, "/issuers.json", payload, &issuer)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, req UpdateRequest) (*UpdateResponse, error) {
	...
	path := fmt.Sprintf("/issuers/%s.json", url.PathEscape(req.ID))
	payload := wrapPayload(req.Input)
	if req.DryRun {
	...
	var issuer map[string]any
	resp, err := client.PutJSON(ctx, path, payload, &issuer)
	if err != nil {
		return nil, err
	}
...
func validateInput(input map[string]any) error {
	if input == nil {
		return output.Usage("missing_input", "issuer input is required", "pass --input -|@file.json|'{...}'")
	}
	return nil
}
...
func wrapPayload(input map[string]any) map[string]any {
	return map[string]any{
		"issuer": cloneValue(input).(map[string]any),
	}
}
